...
/* data written to sensor.file.txt !!! first one needs to be without the + !!! */

string CarState::writeSensor()
{
	string str;
	str = SimpleParser::stringify("track", this->track, TRACK_SENSORS_NUM);
	return str;        
}
string CarState::writeTester()
{
	string str;
	str  = SimpleParser::stringify("rpm", this->rpm);
	str += SimpleParser::stringify("gear", this->gear);
...
int CarState::calculator()
{	
	int speed;
	speed=distRaced/(curLapTime*3,6);
	return speed;        
}
